...
- **Туннель возвращается после гибели процесса ([§428](docs/spec/tasks/428-vpn-service-start-sticky.md), issue #115).**
  Сервис теперь `START_STICKY`: если процесс убил OOM, lmkd или OEM-чистилка,
  система пересоздаёт его сама и VPN поднимается без открытия приложения, в
  том числе под Always-on. Ручной Stop не воскрешается. Предохранитель: не
  больше двух автоперезапусков за пять минут, дальше остановка с уведомлением.
